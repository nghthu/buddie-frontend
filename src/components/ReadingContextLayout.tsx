...
export default function ReadingContextLayout(props: {
  ...
  let matches;
  while (
    (matches = regex.exec(context)) !== null &&
    imageIndexs.length < props.images.length
  ) {
    imageIndexs.push(matches.index + 5);
  }
  for (let i = 0; i < imageIndexs.length; i++) {
    context =
      context.slice(0, imageIndexs[i]) +
      `<img src=${props.images[i]} alt="context-image" />` +
      context.slice(imageIndexs[i]);
  }
  return (
