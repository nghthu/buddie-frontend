...
export default function ReadingLayout(props: {
  ...
  const questionGroups = props.data['question_groups'].map(
    (questionGroup: questiongroup, index: number) => {
      const questions = questionGroup['questions'].map((question: question, index2:number) => {
        const question_use_for_answer = props.answers?.parts[props.partNumber - 1]?.question_groups[index]?.questions[index2];
        return (
          <React.Fragment key={question['question_number']}>
            {question['question_type'] === 'single_choice' && (
              <SingleChoiceLayout
                ...
                userAnswer={question_use_for_answer?.answer_result.user_answer}
