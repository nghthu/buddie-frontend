...
const IeltsReading = () => {
    ...
    return (
      <>
        ...
        <div style={{ paddingLeft: "8%", paddingRight: "8%", fontSize: "2rem" }}>
          <TextCard width="100%" height="auto" className={textCardStyles["card_background-color_very-light-grey"]}>
          ...
          <TextCard width="100%" height="auto" className={clsx(textCardStyles["card_background-color_white"], textCardStyles["cardFlex_Justify_center"])}>
            ...
            <Button className={styles.webButton} type={"primary"} onClick={() => changePart("1")}>Bắt đầu</Button>
          </TextCard>
        </div>
      </>
    );
  ...
  const parts = jsonData["parts"].map((part: any) => {
    ...
    const nextPart = Math.min(part["part_number"] + 1, jsonData["parts"].length);
    return (
      <>
      {activePart === String(part["part_number"]) &&
        <>
          <ReadingLayout paddingLeft={"2%"} paddingRight={"2%"} setPrevState={() => changePart(String(prevPart))} setNextState={() => changePart(String(nextPart))} data={part} setAnswer={()=>setAnswers({...answers})}/>
        </>}
      </>
    );
  });
  return (
    <>
    <SkillHeader title={"Luyện tập IELTS Reading"} countdownTime={testTime} />
    {parts}
    </>
  );
};
